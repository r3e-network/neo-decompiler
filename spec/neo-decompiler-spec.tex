...
\author{Jimmy \texttt{jimmy@r3e.network}}
\date{\today}
...
\begin{itemize}[nosep]
    ...
    \item Unsupported or unknown opcodes shall, by default, be represented with
    mnemonic placeholders (\texttt{UNKNOWN\_0x..}) without aborting disassembly.
    A fail-fast mode shall be available via the CLI or library to error on the
    first unknown opcode.
...
\texttt{disasm} & Instruction listing & Text, JSON; optional fail-fast on unknown opcodes \\
\texttt{decompile} & Pseudocode / High-level / C\# & Text (pseudocode/high-level/C\#), JSON; optional fail-fast on unknown opcodes \\
...
instructions & Array of decoded instructions with operands; \texttt{returns\_value} may be present for \texttt{SYSCALL}.\\
method\_tokens & Tokens with hashes, call flags, \texttt{returns\_value}, and native hints.\\
...
analysis & Best-effort analysis results including call graph, slot cross-references, and inferred types.\\
\bottomrule
\end{longtable}
...
Unknown opcode handling & Default tolerant mode emits \texttt{UNKNOWN\_0x..}; fail-fast mode available via CLI/library.\\
Syscall metadata & Syscalls supply names, call flags, and \texttt{returns\_value} to avoid phantom temporaries.\\
\bottomrule
\end{tabular}
...
\begin{itemize}[nosep]
    \item Type inference is best-effort; values may remain \texttt{unknown} and
    syscall argument signatures are not recovered from bytecode.
    ...
    placeholders in tolerant mode and not lifted; fail-fast mode will error at
