...
use crate::common::{errors::CodeGenerationError, config::{DecompilerConfig, SyntaxStyle}, types::*};
use crate::core::ir::*;
use crate::analysis::types::{Type, PrimitiveType};
...
trait SyntaxFormatter {
    fn format_function_signature(&self, function: &IRFunction, generator: &PseudocodeGenerator) -> String;
    ...
    fn format_function_call(&self, name: &str, args: &[String], return_type: Option<&Type>) -> String;
...
impl PseudocodeGenerator {
    ...
    fn initialize_syscall_names(&mut self) {
        ...
        self.syscall_names.insert(0x49de7d57, "System.Runtime.Platform".to_string());
        self.syscall_names.insert(0x2d43a8aa, "System.Runtime.GetTrigger".to_string());
        self.syscall_names.insert(0xf827ec8e, "System.Runtime.GetTime".to_string());
        self.syscall_names.insert(0x5d97c1b2, "System.Runtime.GetExecutingScriptHash".to_string());
        self.syscall_names.insert(0x91f9b23b, "System.Runtime.GetCallingScriptHash".to_string());
        self.syscall_names.insert(0x9e29b9a8, "System.Runtime.GetEntryScriptHash".to_string());
        self.syscall_names.insert(0x9c7c9598, "System.Storage.GetContext".to_string());
        self.syscall_names.insert(0xe1c83c39, "System.Storage.GetReadOnlyContext".to_string());
        self.syscall_names.insert(0x925de831, "System.Storage.Get".to_string());
        self.syscall_names.insert(0xe63f1884, "System.Storage.Put".to_string());
        self.syscall_names.insert(0x7ce2e494, "System.Storage.Delete".to_string());
        self.syscall_names.insert(0xa09b1eef, "System.Storage.Find".to_string());
        self.syscall_names.insert(0x627d5b52, "System.Contract.Call".to_string());
        self.syscall_names.insert(0x14e12327, "System.Contract.CallEx".to_string());
        self.syscall_names.insert(0x82958f5a, "System.Crypto.CheckSig".to_string());
        self.syscall_names.insert(0xf60652e8, "System.Crypto.CheckMultisig".to_string());
        self.syscall_names.insert(0x7e6a2bb7, "System.Iterator.Next".to_string());
        self.syscall_names.insert(0x63b6c5ee, "System.Iterator.Value".to_string());
        self.syscall_names.insert(0xa0ab5461, "System.Json.Serialize".to_string());
        self.syscall_names.insert(0x7d4b2a25, "System.Json.Deserialize".to_string());
    }
    ...
    pub fn generate(&mut self, function: &IRFunction) -> Result<String, CodeGenerationError> {
        ...
        if self.config.output.include_ir_comments {
            output.push_str(&self.format_comment(&format!("Function: {} (Complexity: cyclomatic={}, blocks={}, operations={})", 
                function.name, 
            ...
            if function.metadata.is_public {
                output.push_str(&self.formatter.format_security_annotation("@public - External entry point"));
                output.push('\n');
            }
            if function.metadata.is_safe {
                output.push_str(&self.formatter.format_security_annotation("@safe - Read-only function"));
                output.push('\n');
            }
        }
    ...
    fn analyze_control_flow(&mut self, function: &IRFunction) -> Result<(), CodeGenerationError> {
        ...
        dfs_visit(function.entry_block, function, &mut visited, &mut in_stack, &mut self.detected_loops)?;
        Ok(())
    }

    /// Generate structured control flow blocks with proper nesting
    fn generate_structured_blocks(&mut self, function: &IRFunction, start_block: BlockId) -> Result<String, CodeGenerationError> {
        let mut output = String::new();
        let mut visited = HashSet::new();
    ...
    fn generate_block_with_structure(&mut self, block: &IRBlock, function: &IRFunction) -> Result<String, CodeGenerationError> {
        let mut output = String::new();
        ...
        if self.config.output.include_ir_comments && block.predecessors.len() > 1 {
            output.push_str(&self.format_indented_comment(&format!("Block {} (predecessors: {:?})", block.id, block.predecessors)));
        }
        ...
        if self.detected_loops.contains(&block.id) {
            ...
            if let Terminator::Branch { condition, true_target, false_target } = &block.terminator {
                let condition_str = self.format_expression(condition)?;
                output.push_str(&self.format_indented(&self.formatter.format_while_loop(&condition_str)));
                self.current_indent += 1;
            }
        }
        ...
        for operation in &block.operations {
            ...
            if !operation_code.is_empty() {
                output.push_str(&self.format_indented(&self.formatter.format_statement(&operation_code)));
            }
        }
    ...
    fn generate_operation(&mut self, operation: &Operation) -> Result<String, CodeGenerationError> {
        match operation {
            ...
            Operation::Syscall { name, arguments, return_type, target } => {
                ...
                let call_str = self.formatter.format_function_call(&resolved_name, &args_str, return_type.as_ref());
            ...
            Operation::ContractCall { contract, method, arguments, call_flags, target } => {
                ...
                let call_str = format!("contract_call({}, \"{}\", [{}], CallFlags(0x{:02x}))", 
                    contract_str, method, args_str.join(", "), call_flags);
            ...
            Operation::Storage { operation: storage_op, key, value, target } => {
                ...
                match storage_op {
                    ...
                    StorageOp::Delete => {
                        Ok(format!("storage_delete({})", key_str))
                    }
                    StorageOp::Find => {
            ...
            Operation::Stack { operation: stack_op, operands, target } => {
                match stack_op {
                    StackOp::Push => {
            ...
            Operation::Arithmetic { operator, left, right, target } => {
            ...
            Operation::Convert { source, target_type, target } => {
                let source_str = self.format_expression(source)?;
            ...
            Operation::Unary { operator, operand, target } => {
                let operand_str = self.format_expression(operand)?;
                let target_str = self.format_variable(target);
            ...
            Operation::BuiltinCall { name, arguments, target } => {
                let args_str = arguments.iter()
                    .map(|arg| self.format_expression(arg))
                    .collect::<Result<Vec<_>, _>>()?;
            ...
            Operation::ArrayOp { operation: array_op, array, index, value, target } => {
            ...
            Operation::MapOp { operation: map_op, map, key, value, target } => {
            ...
            Operation::StringOp { operation: string_op, operands, target } => {
                let target_str = self.format_variable(target);
                match string_op {
                    StringOperation::Concat => {
                        if operands.len() >= 2 {
                            let operand_strs = operands.iter()
                    ...
                    StringOperation::Substring => {
                        if operands.len() >= 3 {
                            ...
                            let expr = format!("{}.substring({}, {})", string_str, start_str, length_str);
                            Ok(self.formatter.format_assignment(&target_str, &expr))
                        } else {
                            Err(CodeGenerationError::InvalidOperation {
                                operation: "String substring requires string, start, length".to_string(),
                            })
                        }
                    }
            ...
            Operation::TypeCheck { value, target_type, target } => {
            ...
            Operation::Effect { effect_type, description } => {
                Ok(self.formatter.format_security_annotation(&format!("@effect {:?}: {}", effect_type, description)))
            }
            
            Operation::Comment(comment) => {
                Ok(self.formatter.format_comment(comment))
            }
        }
    }

    /// Generate structured terminator with control flow awareness
    fn generate_structured_terminator(&mut self, terminator: &Terminator, function: &IRFunction) -> Result<String, CodeGenerationError> {
        match terminator {
            ...
            Terminator::Branch { condition, true_target, false_target } => {
                ...
                if self.detected_loops.contains(&true_target) || self.detected_loops.contains(&false_target) {
                    ...
                    self.current_indent += 1;
                    result.push_str(&self.format_indented_comment(&format!("true branch -> block_{}", true_target)));
                    self.current_indent -= 1;
                    ...
                    self.current_indent += 1;
                    result.push_str(&self.format_indented_comment(&format!("false branch -> block_{}", false_target)));
                    self.current_indent -= 1;
            ...
            Terminator::Switch { discriminant, targets, default_target } => {
                let disc_str = self.format_expression(discriminant)?;
                let mut result = self.format_indented(&format!("switch ({}) {}", disc_str, self.formatter.get_block_start()));
                result.push('\n');
                ...
                for (literal, target) in targets {
                    ...
                    result.push_str(&self.format_indented(&format!("case {}: goto block_{};", case_str, target)));
                }
                
                if let Some(default) = default_target {
                    result.push_str(&self.format_indented(&format!("default: goto block_{};", default)));
                }
            ...
            Terminator::TryBlock { try_block, catch_block, finally_block } => {
                let mut result = self.format_indented(&format!("try {}", self.formatter.get_block_start()));
                result.push('\n');
                
                self.current_indent += 1;
                result.push_str(&self.format_indented_comment(&format!("try block -> block_{}", try_block)));
                self.current_indent -= 1;
                ...
                if let Some(catch) = catch_block {
                    ...
                    self.current_indent += 1;
                    result.push_str(&self.format_indented_comment(&format!("catch block -> block_{}", catch)));
                    self.current_indent -= 1;
                ...
                if let Some(finally) = finally_block {
                    ...
                    self.current_indent += 1;
                    result.push_str(&self.format_indented_comment(&format!("finally block -> block_{}", finally)));
                    self.current_indent -= 1;
        ...
        match expr {
            ...
            Expression::Call { function, arguments } => {
                let args_str = arguments.iter()
                    .map(|arg| self.format_expression(arg))
                    .collect::<Result<Vec<_>, _>>()?;
                Ok(self.formatter.format_function_call(function, &args_str, None))
            }
            ...
            Expression::Struct { fields } => {
                let fields_str = fields.iter()
                    .map(|(name, expr)| {
                        Ok(format!("{}: {}", name, self.format_expression(expr)?))
                    })
                    .collect::<Result<Vec<_>, _>>()?;
    ...
    fn format_type(&self, type_ref: &Type) -> String {
        match type_ref {
            ...
            Type::Map { key, value } => format!("Map<{}, {}>", self.format_type(key), self.format_type(value)),
            Type::Buffer => "Buffer".to_string(),
            Type::Struct(struct_type) => {
                struct_type.name.clone().unwrap_or_else(|| "Struct".to_string())
            },
            Type::Union(types) => {
            ...
            Type::Function { parameters, return_type } => {
                let param_strs: Vec<String> = parameters.iter().map(|p| self.format_type(p)).collect();
    ...
    fn format_variable(&mut self, var: &Variable) -> String {
        ...
        if formatted_name != var.name {
            self.variable_names.insert(var.name.clone(), formatted_name.clone());
            self.var_counter += 1;
        }
...
impl SyntaxFormatter for CStyleFormatter {
    ...
    fn format_block_start(&self, _block_id: BlockId) -> String { "{".to_string() }
    fn format_block_end(&self) -> String { "}".to_string() }
    fn format_statement(&self, statement: &str) -> String { format!("{}{}", statement, self.get_line_ending()) }
    fn format_assignment(&self, target: &str, source: &str) -> String { format!("{} = {}", target, source) }
    fn format_function_call(&self, name: &str, args: &[String], _return_type: Option<&Type>) -> String {
        format!("{}({})", name, args.join(", "))
    }
    fn format_if_statement(&self, condition: &str, _generator: &PseudocodeGenerator) -> String {
        format!("if ({})", condition)
    }
    fn format_while_loop(&self, condition: &str) -> String { format!("while ({})", condition) }
    fn format_return(&self, expr: Option<&str>) -> String {
        if let Some(e) = expr {
            format!("return {}", e)
        } else {
    ...
    fn format_comment(&self, text: &str) -> String { format!("// {}", text) }
    fn format_security_annotation(&self, annotation: &str) -> String {
        format!("/* {} */", annotation)
    }
    fn get_line_ending(&self) -> &'static str { ";" }
...
impl SyntaxFormatter for PythonFormatter {
    ...
    fn format_block_start(&self, _block_id: BlockId) -> String { ":".to_string() }
    fn format_block_end(&self) -> String { "".to_string() }
    fn format_statement(&self, statement: &str) -> String { statement.to_string() }
    fn format_assignment(&self, target: &str, source: &str) -> String { format!("{} = {}", target, source) }
    fn format_function_call(&self, name: &str, args: &[String], _return_type: Option<&Type>) -> String {
        format!("{}({})", name, args.join(", "))
    }
...
impl SyntaxFormatter for RustFormatter {
    ...
    fn format_block_end(&self) -> String { "}".to_string() }
    fn format_statement(&self, statement: &str) -> String { format!("{}{}", statement, self.get_line_ending()) }
    fn format_assignment(&self, target: &str, source: &str) -> String { format!("let {} = {}", target, source) }
    fn format_function_call(&self, name: &str, args: &[String], _return_type: Option<&Type>) -> String {
        format!("{}({})", name, args.join(", "))
    }
    fn format_if_statement(&self, condition: &str, _generator: &PseudocodeGenerator) -> String {
...
mod tests {
    ...
    fn test_format_literal() {
        ...
        assert_eq!(generator.format_literal(&Literal::String("test".to_string())), "\"test\"");
        assert_eq!(generator.format_literal(&Literal::ByteArray(vec![0x12, 0x34])), "0x1234");
    ...
    fn test_format_binary_operator() {
        ...
        assert_eq!(generator.format_binary_operator(BinaryOperator::Equal), "==");
        assert_eq!(generator.format_binary_operator(BinaryOperator::BoolAnd), "&&");
    }
    ...
    fn test_format_type() {
        ...
        assert_eq!(generator.format_type(&Type::Primitive(PrimitiveType::Boolean)), "bool");
        assert_eq!(generator.format_type(&Type::Primitive(PrimitiveType::Integer)), "integer");
        assert_eq!(generator.format_type(&Type::Array(Box::new(Type::Primitive(PrimitiveType::Integer)))), "Array<integer>");
    }
    ...
    fn test_complex_operations() {
        ...
        let contract_call = Operation::ContractCall {
            ...
            arguments: vec![
                Expression::Variable(Variable { name: "from".to_string(), id: 1, var_type: VariableType::Parameter }),
                Expression::Variable(Variable { name: "to".to_string(), id: 2, var_type: VariableType::Parameter }),
                Expression::Literal(Literal::Integer(100)),
            ],
            call_flags: 0x01,
            target: Some(Variable { name: "success".to_string(), id: 3, var_type: VariableType::Local }),
        };
    ...
    fn test_expression_formatting() {
        ...
        let binary_expr = Expression::BinaryOp {
            left: Box::new(Expression::Variable(Variable { name: "a".to_string(), id: 1, var_type: VariableType::Local })),
    ...
    fn test_terminator_formatting() {
        ...
        let result = generator.generate_structured_terminator(&return_term, &function).unwrap();
        assert!(result.contains("return"));
        assert!(result.contains("42"));
        
        // Test branch terminator
        let branch_term = Terminator::Branch {
            condition: Expression::Variable(Variable { name: "condition".to_string(), id: 1, var_type: VariableType::Local }),
            true_target: 1,
            false_target: 2,
        };
        let branch_result = generator.generate_structured_terminator(&branch_term, &function).unwrap();
