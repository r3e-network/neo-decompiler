...
mod tests {
    use crate::{
        ...
        core::{syscalls::SyscallDatabase, lifter::IRLifter, decompiler::DecompilerEngine},
        analysis::types::{Type, PrimitiveType},
    };
    ...
    fn test_syscall_database_integration() {
        ...
        assert_eq!(db.resolve_name(0x5d97c1b2), "System.Runtime.GetExecutingScriptHash");
    ...
    fn test_syscall_type_signatures() {
        ...
        let sig = db.get_signature(0x925de831).expect("Should have Storage.Get signature");
        ...
        let sig = db.get_signature(0xe63f1884).expect("Should have Storage.Put signature");
        assert_eq!(sig.name, "System.Storage.Put");
        assert_eq!(sig.parameters.len(), 3);
        assert!(matches!(sig.return_type, Type::Void));

        // Test pure syscall
        let sig = db.get_signature(0x5d97c1b2).expect("Should have GetExecutingScriptHash signature");
        assert_eq!(sig.name, "System.Runtime.GetExecutingScriptHash");
        assert_eq!(sig.parameters.len(), 0);
        assert!(matches!(sig.return_type, Type::Primitive(PrimitiveType::ByteArray)));
    }
    ...
    fn test_all_major_neo_syscalls_present() {
        ...
        for (hash, expected_name) in major_syscalls {
            assert_eq!(db.resolve_name(hash), expected_name, 
                "Syscall 0x{:08x} should resolve to {}", hash, expected_name);
        }
    }
}