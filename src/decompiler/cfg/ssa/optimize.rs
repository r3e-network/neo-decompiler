...
mod tests {
    ...
    fn retargets_terminator_use_when_removing_variable_trivial_phi() {
        ...
        predecessor.add_stmt(assign_str(
            incoming.clone(),
            SsaExpr::call("condition".to_string(), vec![]),
        ));
    ...
    fn rewrites_expression_through_variable_trivial_phi_chain() {
        ...
        inner_phi.add_operand(BlockId(1), incoming.clone());
        merge.add_phi(inner_phi);
        merge.add_stmt(SsaStmt::expr(SsaExpr::call(
            "use".to_string(),
        ...
        predecessor.add_stmt(assign_str(
            incoming.clone(),
            SsaExpr::call("value".to_string(), vec![]),
        ));
        ...
        let effect = ssa
            ...
            .find_map(|stmt| match stmt {
                SsaStmt::Expr(SsaExpr::Call { name, args }) if name == "use" => Some(args),
                _ => None,
            })
            .expect("effect use");
    ...
    fn preserves_literal_trivial_phi_used_by_nontrivial_phi() {
        ...
        other_predecessor.add_stmt(assign_str(
            other,
            SsaExpr::call("other".to_string(), vec![]),
        ));
    ...
    fn effect_statement_keeps_input_definition_live() {
        ...
        assert!(
            block.stmts.iter().any(|stmt| matches!(
                ...
                } if target == &input && name == "newmap" && args.is_empty()
            )),
        ...
        let effect_index = block
            ...
            .position(|stmt| {
                matches!(
                    stmt,
                    SsaStmt::Expr(SsaExpr::Call { name, .. }) if name == "set_item"
                )
            })
