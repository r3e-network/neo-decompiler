...
fn compute_postdominators(cfg: &Cfg) -> BTreeMap<BlockId, BTreeSet<BlockId>> {
...
fn find_irreducible_region(cfg: &Cfg) -> Option<BTreeSet<BlockId>> {
    for block in cfg.blocks() {
...
fn reachable_cfg(cfg: &Cfg, start: BlockId) -> BTreeSet<BlockId> {
...
fn reverse_reachable_cfg(cfg: &Cfg, start: BlockId) -> BTreeSet<BlockId> {
    let mut seen = BTreeSet::new();
    let mut stack = vec![start];
    while let Some(block) = stack.pop() {
        if !seen.insert(block) {
...
fn simplify_unreachable_control(block: &mut IrBlock) {
    loop {
...
impl<'a> StructCtx<'a> {
    fn structure_irreducible(&self, entry: BlockId, region: &BTreeSet<BlockId>) -> IrBlock {
