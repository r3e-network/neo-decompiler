...
use super::plan::csharp_type_value_type;

mod bytes;
use bytes::{render_byte_concat, render_byte_slice, render_memcpy};
...
pub(super) fn render_intrinsic(
    ...
    match opcode {
        ...
        OpCode::Isnull => {
            if args
                .first()
                .is_some_and(|value| definitely_non_null_value(value, context))
            {
...
fn definitely_non_null_value(expression: &Expr, context: &ExprContext) -> bool {
    match expression {
