...
fn assert_pinned_incomplete_baseline(root: &Path, incomplete_locations: &BTreeSet<String>) {
    ...
    // Offset-bearing baseline: valid only for the pinned v3.10.0 corpus. A
    // newer devpack checkout shifts these offsets and silently invalidates
    // them, so never re-baseline from an unpinned `../neo-devpack-dotnet`.
