...
pub fn demonstrate_cfg_analysis() -> Result<(), CFGError> {
    ...
    if !cfg.loops.is_empty() {
        ...
        for (i, loop_info) in cfg.loops.iter().enumerate() {
            println!("     Loop {}: header={}, body_size={}, type={:?}", 
    ...
    println!("   - Exception regions: {}", exception_cfg.exception_regions.len());
    ...
    if !exception_cfg.exception_regions.is_empty() {
        for (i, region) in exception_cfg.exception_regions.iter().enumerate() {
            println!("   - Region {}: protected={}, handlers={}, finally={}", 
                    i, region.protected_blocks.len(), region.handler_blocks.len(), 
    ...
    let switch_edges: Vec<_> = switch_cfg.edges.iter()
        .filter(|e| matches!(e.edge_type, EdgeType::SwitchCase(_) | EdgeType::SwitchDefault))
        .collect();
    ...
    println!("   ✓ DOT export generated ({} characters)", dot_output.len());
    ...
    match cfg.topological_sort() {
        ...
        Err(CFGError::CyclicDependency) => println!("   - Topological sort: Not applicable (cyclic graph)"),
    ...
    println!("   - Full analysis: {} complexity metrics computed", 
             if cfg.complexity.cyclomatic_complexity > 0 { "✓" } else { "✗" });
    println!("   - Minimal analysis: {} complexity metrics computed", 
             if minimal_cfg.complexity.cyclomatic_complexity > 0 { "✓" } else { "✗" });
...
mod tests {
    ...
    fn test_exception_cfg_demo() {
        ...
        assert!(!cfg.exception_regions.is_empty(), "Should detect exception regions");
        
        // Verify edge types
        let try_edges: Vec<_> = cfg.edges.iter()
            .filter(|e| matches!(e.edge_type, EdgeType::TryEntry | EdgeType::CatchEntry | EdgeType::FinallyEntry))
            .collect();
    ...
    fn test_switch_cfg_demo() {
        ...
        let switch_edges: Vec<_> = cfg.edges.iter()
            .filter(|e| matches!(e.edge_type, EdgeType::SwitchCase(_) | EdgeType::SwitchDefault))
            .collect();
        assert_eq!(switch_edges.len(), 4, "Should have 3 case edges + 1 default edge");
    }
