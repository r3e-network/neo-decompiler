...
impl DecompilerEngine {
    ...
    pub fn with_optimization_level(config: &DecompilerConfig, opt_level: OptimizationLevel) -> Self {
    ...
    pub fn analyze(
        ...
        if self.config.analysis.enable_type_inference {
            ...
            let type_results = self.apply_type_inference(ir_function, results.cfg.as_ref(), manifest)?;
        ...
        if self.config.analysis.enable_effect_analysis {
            ...
            let effect_results = self.apply_effect_analysis(ir_function, results.cfg.as_ref(), &results.type_results)?;
    ...
    fn initialize_manifest_types(
        ...
        for method in &manifest.abi.methods {
            ...
            context.function_types.insert(format!("method_{}", method.name), method_type);
        }
        
        // Store event signatures
        for event in &manifest.abi.events {
            let event_fields: Vec<crate::analysis::types::FieldType> = event.parameters.iter()
                .enumerate()
            ...
            context.function_types.insert(format!("event_{}", event.name), event_type);
        }
        Ok(())
    }
    ...
    fn generate_operation_constraints(
        ...
        match operation {
            ...
            Operation::Arithmetic { operator, left, right, target } => {
                ...
                let result_type = self.infer_arithmetic_result_type(operator, &left_type, &right_type);
                let target_var_name = self.variable_name(target)?;
            ...
            Operation::Convert { source, target_type, target } => {
                ...
                context.add_constraint(TypeConstraint::Convertible(source_type, target_type.clone()));
                context.set_variable_type(target_var_name, target_type.clone());
            }
            
            Operation::ArrayOp { operation, array, index, value, target } => {
            ...
            Operation::Storage { operation, key, value, target } => {
                    ...
                    StorageOp::Get => {
                        if let Some(target) = target {
                            ...
                            context.set_variable_type(
                                target_var_name,
                                Type::Nullable(Box::new(Type::Primitive(crate::analysis::types::PrimitiveType::ByteString)))
                            );
                        }
                    }
    ...
    fn infer_expression_type(
        ...
        match expr {
            ...
            Expression::Variable(var) => {
                ...
                Ok(context.get_variable_type(&var_name)
                    .cloned()
                    .unwrap_or(Type::Unknown))
            }
            
            Expression::BinaryOp { op: operator, left, right } => {
                let left_type = self.infer_expression_type(context, left)?;
                let right_type = self.infer_expression_type(context, right)?;
                Ok(self.infer_arithmetic_result_type(operator, &left_type, &right_type))
            }
            
            Expression::UnaryOp { op: operator, operand } => {
    ...
    fn literal_type(&self, literal: &Literal) -> Type {
        match literal {
            ...
            Literal::BigInteger(_) => Type::Primitive(crate::analysis::types::PrimitiveType::Integer),
            Literal::String(_) => Type::Primitive(crate::analysis::types::PrimitiveType::ByteString),
            Literal::ByteArray(_) => Type::Primitive(crate::analysis::types::PrimitiveType::ByteString),
    ...
    fn infer_arithmetic_result_type(&self, _operator: &BinaryOperator, left_type: &Type, right_type: &Type) -> Type {
    ...
    fn infer_unary_result_type(&self, operator: &UnaryOperator, operand_type: &Type) -> Type {
        match operator {
            ...
            UnaryOperator::BoolNot => Type::Primitive(crate::analysis::types::PrimitiveType::Boolean),
    ...
    fn convert_side_effect_to_effect(&self, side_effect: &crate::analysis::types::SideEffect, syscall_name: &str) -> Effect {
    ...
    fn solve_type_constraints(&self, context: &mut TypeInferenceContext) -> Result<(), AnalysisError> {
    ...
    fn apply_constraint(&self, context: &mut TypeInferenceContext, constraint: &TypeConstraint) -> Result<bool, AnalysisError> {
        match constraint {
            TypeConstraint::Equal(t1, t2) => {
                self.unify_types(context, t1, t2)
            }
    ...
    /// Unify two types
    fn unify_types(&self, context: &mut TypeInferenceContext, t1: &Type, t2: &Type) -> Result<bool, AnalysisError> {
        if t1 == t2 {
        ...
        match (t1, t2) {
            ...
            (Type::Array(elem1), Type::Array(elem2)) => {
                self.unify_types(context, elem1, elem2)
            }
    ...
    fn count_applied_passes(&self) -> u32 {
        ...
        if self.config.analysis.enable_type_inference { count += 1; }
        if self.config.analysis.enable_cfg_analysis { count += 1; }
        if self.config.analysis.enable_effect_analysis { count += 1; }
        if self.config.analysis.enable_loop_detection { count += 1; }
        if self.config.analysis.enable_dead_code_elimination { count += 1; }
    ...
    fn analyze_operation_effects(
        ...
        match operation {
            ...
            Operation::Storage { operation: storage_op, key, .. } => {
                let key_pattern = self.extract_key_pattern(key)?;
                match storage_op {
                    StorageOp::Get => {
                        Ok(vec![Effect::StorageRead { key_pattern }])
                    }
                    StorageOp::Put => {
                        Ok(vec![Effect::StorageWrite { key_pattern }])
                    }
                    StorageOp::Delete => {
                        Ok(vec![Effect::StorageWrite { key_pattern }])
                    }
            ...
            Operation::ContractCall { contract, method, .. } => {
    ...
    fn analyze_terminator_effects(&self, terminator: &Terminator) -> Result<Vec<Effect>, AnalysisError> {
        match terminator {
            Terminator::Abort(_) => {
    ...
    fn extract_key_pattern(&self, key_expr: &Expression) -> Result<KeyPattern, AnalysisError> {
        match key_expr {
            Expression::Literal(Literal::String(s)) => {
                Ok(KeyPattern::Exact(s.as_bytes().to_vec()))
            }
            Expression::Literal(Literal::ByteArray(bytes)) => {
                Ok(KeyPattern::Exact(bytes.clone()))
            }
        ...
        match contract_expr {
            ...
            Expression::Literal(Literal::String(hash_str)) => {
                ...
                if hash_str.len() == 40 || (hash_str.len() == 42 && hash_str.starts_with("0x")) {
                    let hex_part = if hash_str.starts_with("0x") { &hash_str[2..] } else { hash_str };
                    
                    let bytes = hex::decode(hex_part)
                        .map_err(|_| AnalysisError::InvalidContractHash { 
                            expected_length: 20, 
    ...
    fn assess_security_risks(
        ...
        let storage_writes: Vec<_> = function_effects.iter()
        ...
        if storage_writes.len() > 5 {
            risks.push(SecurityRisk {
                ...
                severity: 6,
                description: "Function performs many storage writes, consider access controls".to_string(),
                affected_blocks: block_effects.keys().cloned().collect(),
                mitigation: Some("Add proper access control checks before storage operations".to_string()),
            });
        }
        
        // Check for potential reentrancy
        let has_contract_calls = function_effects.iter().any(|e| matches!(e, Effect::ContractCall { .. }));
        ...
        if has_contract_calls && has_storage_writes {
            risks.push(SecurityRisk {
                ...
                severity: 8,
                description: "Function makes external calls and modifies storage, potential reentrancy".to_string(),
                affected_blocks: vec![],
                mitigation: Some("Use checks-effects-interactions pattern or reentrancy guards".to_string()),
            });
        }
        
        // Check for gas consumption issues
        let total_gas: u64 = function_effects.iter()
            .filter_map(|e| match e {
    ...
    fn apply_cfg_analysis(&mut self, ir_function: &IRFunction) -> Result<ControlFlowGraph, AnalysisError> {
        // Build comprehensive CFG with advanced analysis
        let cfg = self.cfg_builder.build_cfg(ir_function)
    ...
    fn apply_loop_detection(&self, ir_function: &mut IRFunction, cfg: &ControlFlowGraph) -> Result<(), AnalysisError> {
    ...
    fn analyze_loop_structure(
        ...
        analysis.is_performance_critical = analysis.body_size > 10 || 
            cfg.complexity.max_loop_depth > 3;
    ...
    fn apply_optimization_passes(
        ...
        opt_results.subexpressions_eliminated = self.apply_common_subexpression_elimination(ir_function)?;
    ...
    fn eliminate_dead_code(
        ...
        ir_function.exit_blocks.retain(|&block_id| !dead_blocks.contains(&block_id));
    ...
    fn apply_loop_optimizations(
        ...
        for loop_info in &cfg.loops {
            ...
            if loop_info.body.len() <= 3 && 
               loop_info.estimated_iterations.unwrap_or(100) <= 4 {
                optimizations.push(LoopOptimization {
                    loop_header: loop_info.header,
        ...
        for block in ir_function.blocks.values() {
            ...
            for operation in &block.operations {
                ...
                let stack_change = match operation {
                    ...
                    Operation::Stack { operation: stack_op, .. } => {
                        match stack_op {
                            StackOp::Push => 1,
                            StackOp::Pop => -1,
                            StackOp::Dup => 1,
        ...
        for block in ir_function.blocks.values() {
            for operation in &block.operations {
                match operation {
                    ...
                    Operation::Storage { operation: StorageOp::Get, .. } => {
    ...
    fn parse_neo_type(&self, type_string: &str) -> Result<Type, AnalysisError> {
        ...
        match trimmed {
            ...
            // Complex types
            type_str if type_str.starts_with("Array") => {
...
mod tests {
    ...
    fn test_parse_neo_type_primitives() {
        ...
        assert_eq!(engine.parse_neo_type("Boolean").unwrap(), Type::Primitive(PrimitiveType::Boolean));
        assert_eq!(engine.parse_neo_type("Integer").unwrap(), Type::Primitive(PrimitiveType::Integer));
        assert_eq!(engine.parse_neo_type("ByteString").unwrap(), Type::Primitive(PrimitiveType::ByteString));
        assert_eq!(engine.parse_neo_type("String").unwrap(), Type::Primitive(PrimitiveType::ByteString)); // String maps to ByteString
        assert_eq!(engine.parse_neo_type("Hash160").unwrap(), Type::Primitive(PrimitiveType::Hash160));
        assert_eq!(engine.parse_neo_type("Hash256").unwrap(), Type::Primitive(PrimitiveType::Hash256));
        assert_eq!(engine.parse_neo_type("PublicKey").unwrap(), Type::Primitive(PrimitiveType::PublicKey));
        assert_eq!(engine.parse_neo_type("Signature").unwrap(), Type::Primitive(PrimitiveType::Signature));
        assert_eq!(engine.parse_neo_type("ECPoint").unwrap(), Type::Primitive(PrimitiveType::ECPoint));
        
        // Test special types
        assert_eq!(engine.parse_neo_type("Any").unwrap(), Type::Any);
        assert_eq!(engine.parse_neo_type("Void").unwrap(), Type::Void);
        assert_eq!(engine.parse_neo_type("Null").unwrap(), Type::Primitive(PrimitiveType::Null));
    ...
    fn test_parse_neo_type_edge_cases() {
        ...
        match engine.parse_neo_type("Array<Map<String, Integer>>").unwrap() {
    ...
    fn test_analyze_with_default_config() {
        ...
        assert!(analysis_results.type_results.variable_types.is_empty() || 
    ...
    fn test_decompiler_with_optimization_levels() {
        ...
        let _engine_none = DecompilerEngine::with_optimization_level(&config, OptimizationLevel::None);
        let _engine_basic = DecompilerEngine::with_optimization_level(&config, OptimizationLevel::Basic);
        let _engine_aggressive = DecompilerEngine::with_optimization_level(&config, OptimizationLevel::Aggressive);
    }
