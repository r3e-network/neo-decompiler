...
use crate::analysis::types::{Type, PrimitiveType, StructType};
...
impl IRLifter {
    ...
    fn build_offset_mapping(&mut self, boundaries: &[u32]) {
        ...
        for (block_id, &start_offset) in boundaries.iter().enumerate() {
            ...
            self.offset_to_block.insert(start_offset, block_id as BlockId);
        ...
        for (block_id, &start_offset) in boundaries.iter().enumerate() {
            ...
            for instruction in instructions {
                if instruction.offset >= start_offset && instruction.offset < end_offset {
                    self.offset_to_block.insert(instruction.offset, block_id as BlockId);
    ...
    fn extract_jump_target(&self, instruction: &Instruction) -> Option<u32> {
        match &instruction.operand {
            Some(Operand::JumpTarget(target)) => {
    ...
    /// Build basic blocks from instructions
    fn build_basic_blocks(&mut self, instructions: &[Instruction], boundaries: &[u32]) -> Result<Vec<IRBlock>, LiftError> {
        let mut blocks = Vec::new();
        
        for (block_id, &start_offset) in boundaries.iter().enumerate() {
            let end_offset = boundaries.get(block_id + 1).copied().unwrap_or(u32::MAX);
            
            let block_instructions: Vec<_> = instructions.iter()
    ...
    fn build_block_relationships(&self, block: &mut IRBlock, instructions: &[&Instruction], boundaries: &[u32]) {
        if let Some(&last_instruction) = instructions.last() {
            ...
            if matches!(block.terminator, Terminator::Jump(0)) && 
               !matches!(last_instruction.opcode, OpCode::RET | OpCode::ABORT | OpCode::ABORTMSG | OpCode::THROW | OpCode::JMP | OpCode::JMP_L) {
    ...
    fn build_predecessor_relationships(&self, blocks: &mut [IRBlock]) {
        ...
        for block in blocks.iter() {
            for &successor_id in &block.successors {
                predecessor_map.entry(successor_id)
    ...
    fn validate_offset_mapping(&self, instructions: &[Instruction]) -> Result<(), LiftError> {
        ...
        for instruction in instructions {
            ...
            if block_id >= 1000 { // Sanity check for block count
                return Err(LiftError::InvalidControlFlow {
                    offset: instruction.offset,
                });
            }
        ...
        for instruction in instructions {
            if let Some(target_offset) = self.extract_jump_target(instruction) {
                if !self.offset_to_block.contains_key(&target_offset) {
                    ...
                    if !instructions.iter().any(|instr| instr.offset == target_offset) {
                        return Err(LiftError::InvalidControlFlow {
                            offset: instruction.offset,
                        });
                    }
    ...
    fn build_block(&mut self, block_id: BlockId, instructions: &[&Instruction]) -> Result<IRBlock, LiftError> {
    ...
    fn lift_instruction(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        ...
        match instruction.opcode {
            ...
            OpCode::EQUAL | OpCode::NUMEQUAL => operations.extend(self.lift_binary_arithmetic(BinaryOperator::Equal)?),
            OpCode::NOTEQUAL | OpCode::NUMNOTEQUAL => operations.extend(self.lift_binary_arithmetic(BinaryOperator::NotEqual)?),
            OpCode::LT => operations.extend(self.lift_binary_arithmetic(BinaryOperator::Less)?),
            OpCode::LE => operations.extend(self.lift_binary_arithmetic(BinaryOperator::LessEqual)?),
            OpCode::GT => operations.extend(self.lift_binary_arithmetic(BinaryOperator::Greater)?),
            OpCode::GE => operations.extend(self.lift_binary_arithmetic(BinaryOperator::GreaterEqual)?),
            
            // Boolean operations  
            OpCode::NOT => operations.extend(self.lift_unary_op(UnaryOperator::BoolNot)?),
            OpCode::BOOLAND => operations.extend(self.lift_binary_arithmetic(BinaryOperator::BoolAnd)?),
            OpCode::BOOLOR => operations.extend(self.lift_binary_arithmetic(BinaryOperator::BoolOr)?),
            ...
            // Type operations
            OpCode::ISNULL => operations.extend(self.lift_type_check(Type::Primitive(PrimitiveType::Null))?),
        ...
        match instruction.opcode {
            OpCode::RET => {
                ...
                let return_value = if !self.stack.is_empty() {
                    Some(self.pop_stack().unwrap_or(Expression::Literal(Literal::Null)))
                } else {
                    None
                };
            ...
            OpCode::THROW => {
                let exception = if !self.stack.is_empty() {
                    self.pop_stack().unwrap_or(Expression::Literal(Literal::Null))
                } else {
            ...
            OpCode::JMP | OpCode::JMP_L => {
                if let Some(target_offset) = self.extract_jump_target(instruction) {
                    let target_block = self.offset_to_block.get(&target_offset).copied().unwrap_or(0);
                    Ok(Terminator::Jump(target_block))
                } else {
                    Err(LiftError::InvalidControlFlow { offset: instruction.offset })
                }
            }
            ...
            OpCode::JMPIF | OpCode::JMPIF_L => {
                if let Some(target_offset) = self.extract_jump_target(instruction) {
                    let true_target = self.offset_to_block.get(&target_offset).copied().unwrap_or(0);
                    let false_target = self.offset_to_block.get(&(instruction.offset + instruction.size as u32)).copied().unwrap_or(0);
                    
                    let condition = if !self.stack.is_empty() {
                        self.pop_stack().unwrap_or(Expression::Literal(Literal::Boolean(true)))
                    } else {
            ...
            OpCode::JMPIFNOT | OpCode::JMPIFNOT_L => {
                if let Some(target_offset) = self.extract_jump_target(instruction) {
                    let true_target = self.offset_to_block.get(&(instruction.offset + instruction.size as u32)).copied().unwrap_or(0);
                    let false_target = self.offset_to_block.get(&target_offset).copied().unwrap_or(0);
            ...
            OpCode::JMPLT | OpCode::JMPLT_L | OpCode::JMPLE | OpCode::JMPLE_L => {
                if let Some(target_offset) = self.extract_jump_target(instruction) {
                    ...
                    let right = if !self.stack.is_empty() {
                        self.pop_stack().unwrap_or(Expression::Literal(Literal::Integer(0)))
                    } else {
            ...
            OpCode::CALL | OpCode::CALL_L => {
                if let Some(target_offset) = self.extract_jump_target(instruction) {
                    ...
                    let return_block = self.offset_to_block.get(&(instruction.offset + instruction.size as u32)).copied().unwrap_or(0);
            ...
            OpCode::CALLA | OpCode::CALLT => {
                ...
                let next_block = self.offset_to_block.get(&(instruction.offset + instruction.size as u32)).copied().unwrap_or(0);
            ...
            OpCode::TRY | OpCode::TRY_L => {
                if let Some(Operand::TryBlock { catch_offset, finally_offset }) = &instruction.operand {
                    let try_block = self.offset_to_block.get(&(instruction.offset + instruction.size as u32)).copied().unwrap_or(0);
                    let catch_block = self.offset_to_block.get(catch_offset).copied();
                    let finally_block = finally_offset.and_then(|offset| self.offset_to_block.get(&offset).copied());
    ...
    fn lift_push_constant(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        match &instruction.operand {
            ...
            _ => return Err(LiftError::InvalidOperand { 
                offset: instruction.offset 
            }),
        }
        Ok(vec![])
    ...
    fn lift_push_address(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
    ...
    fn lift_binary_arithmetic(&mut self, operator: BinaryOperator) -> Result<Vec<Operation>, LiftError> {
    ...
    fn lift_increment(&mut self, delta: i64) -> Result<Vec<Operation>, LiftError> {
        ...
        let operation = Operation::Arithmetic {
            operator: if delta > 0 { BinaryOperator::Add } else { BinaryOperator::Sub },
            left: operand,
            right: increment,
            target: target.clone(),
        };
    ...
    fn lift_builtin_call(&mut self, name: &str, arg_count: usize) -> Result<Vec<Operation>, LiftError> {
    ...
    fn lift_load_local_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
            self.lift_load_local(*slot as u32)
        } else {
            Err(LiftError::InvalidOperand { offset: instruction.offset })
        }
    }
    
    /// Lift store local with operand
    fn lift_store_local_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
    ...
    fn lift_load_arg_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
    ...
    fn lift_store_arg_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
    ...
    fn lift_load_static_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
    ...
    fn lift_store_static_operand(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotIndex(slot)) = &instruction.operand {
    ...
    fn lift_init_static_slot(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::SlotInit { static_slots, .. }) = &instruction.operand {
            Ok(vec![Operation::Comment(
                format!("Initialize {} static slots", static_slots)
            )])
        } else {
            Ok(vec![])
        }
    ...
    fn lift_new_buffer(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::BufferSize(size)) = &instruction.operand {
            self.push_stack(Expression::Literal(Literal::ByteArray(vec![0; *size as usize])));
        }
        Ok(vec![])
    }
    ...
    fn lift_pack_struct(&mut self, instruction: &Instruction) -> Result<Vec<Operation>, LiftError> {
        if let Some(Operand::Count(count)) = &instruction.operand {
            ...
            items.reverse();
            self.push_stack(Expression::StructCreate {
                fields: items.into_iter().enumerate()
...
mod tests {
    ...
    fn create_test_instruction(opcode: OpCode, offset: u32, operand: Option<Operand>) -> Instruction {
        Instruction {
            offset,
            opcode,
            operand,
