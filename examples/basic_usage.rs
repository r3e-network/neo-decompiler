...
fn basic_decompilation_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "📄 Generated pseudocode ({} chars):",
        result.pseudocode.len()
    );
...
fn component_usage_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "🏷️ Magic: {:?}",
        std::str::from_utf8(&nef_file.magic).unwrap_or("N/A")
    );
...
fn main() {
    ...
    let examples = [
        (
            "Basic Decompilation",
            basic_decompilation_example as fn() -> Result<(), Box<dyn std::error::Error>>,
        ),
