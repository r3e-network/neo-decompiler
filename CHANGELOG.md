...
- **Analysis: an out-of-range call target fabricated a spurious call edge and
  method.** A `CALL`/`CALL_L` (or a `PUSHA`+`CALLA` pointer) whose relative
  target lands past the script end — or off an instruction boundary — was
  treated as a resolvable internal call, so the `--format json` `call_graph`
  showed a real-looking `Internal` edge to a non-existent `sub_0xNNNN` method
  (and the `MethodTable` even added a phantom method span for it) — asymmetric
  with the negative-target path, which already reported `UnresolvedInternal`.
  Both ports now require the target to land on a valid instruction offset:
  `CALL`/`CALL_L` reports `UnresolvedInternal` and `CALLA` reports `Indirect`,
  and no phantom method is created. (Only malformed/adversarial bytecode
  triggers this; a conforming compiler never emits an out-of-range call.)
