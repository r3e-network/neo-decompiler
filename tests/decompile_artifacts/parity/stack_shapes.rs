...
fn foreach_pack_helpers_do_not_emit_literal_pack_underflow_warnings() {
    ...
    assert!(
        ...
        "PACK placeholder modeling should not regress the helper's STLOC4 stack check: {high_level}"
    );
    assert!(
...
fn foreach_tuple_helper_underflow_stays_explicit_and_compile_safe() {
    ...
    assert!(
        method.contains(
            "sub_0x04A4((dynamic)(((object)null) ?? throw new InvalidOperationException(\"VM argument underflow"
        ),
