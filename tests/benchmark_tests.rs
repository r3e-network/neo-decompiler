...
fn bench_disassembly(c: &mut Criterion) {
    ...
    for (name, bytecode) in bytecode_samples {
        ...
        group.bench_with_input(BenchmarkId::new("disassemble", name), &bytecode, |b, code| {
            b.iter(|| {
...
fn bench_ir_lifting(c: &mut Criterion) {
    ...
    for (name, bytecode) in bytecode_samples {
        ...
        group.bench_with_input(BenchmarkId::new("lift", name), &instructions, |b, instrs| {
            b.iter(|| {
...
fn bench_end_to_end_decompilation(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("simple_with_manifest", SampleNefData::minimal(), Some(SampleManifest::simple_contract())),
        ("nep17_contract", SampleNefData::with_control_flow(), Some(SampleManifest::nep17_token())),
    ];
    ...
    for (name, sample_nef, sample_manifest) in test_cases {
        ...
        group.throughput(Throughput::Bytes(nef_bytes.len() as u64));
        group.bench_with_input(BenchmarkId::new("decompile", name), &(nef_bytes, manifest_json), 
            |b, (nef_data, manifest)| {
                b.iter(|| {
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    for (name, bytecode) in large_bytecode_samples {
        ...
        group.throughput(Throughput::Bytes(bytecode.len() as u64));
        group.bench_with_input(BenchmarkId::new("full_pipeline", name), &bytecode, |b, code| {
            b.iter(|| {
