...
fn test_end_to_end_decompilation_minimal() {
    ...
    assert!(
        !decompiled.pseudocode.is_empty(),
        "Should generate non-empty pseudocode"
    );
    assert!(
        !decompiled.instructions.is_empty(),
        "Should have disassembled instructions"
    );
    assert!(
        decompiled.manifest.is_some(),
        "Should include parsed manifest"
    );
    assert_eq!(
        decompiled.nef_file.magic, *b"NEF3",
        "Should preserve NEF magic"
    );
}
...
fn test_end_to_end_nep17_decompilation() {
    ...
    assert!(
        pseudocode.contains("transfer")
            || pseudocode.contains("balanceOf")
            || pseudocode.len() > 50,
        "Should contain NEP-17 related content"
    );
}
...
fn test_decompilation_without_manifest() {
    ...
    assert!(
        result.is_ok(),
        "Decompilation without manifest should succeed"
    );

    let decompiled = result.unwrap();
    assert!(
        !decompiled.pseudocode.is_empty(),
        "Should generate pseudocode without manifest"
    );
    assert!(decompiled.manifest.is_none(), "Manifest should be None");
    assert!(
        !decompiled.instructions.is_empty(),
        "Should still disassemble instructions"
    );
}
...
fn test_multiple_output_formats() {
    ...
    let result = decompiler
        .decompile(&nef_bytes, Some(&manifest_json))
        .unwrap();
    ...
    assert!(
        !pseudocode.is_empty(),
        "Basic pseudocode should not be empty"
    );
    ...
    assert!(
        html_output.contains(&result.pseudocode) || html_output.contains("&lt;"),
        "Should contain escaped pseudocode"
    );
}
...
fn test_configuration_integration() {
    ...
    assert!(
        result.is_ok(),
        "Decompilation with custom config should succeed"
    );
    ...
    assert!(
        !decompiled.pseudocode.is_empty(),
        "Should generate optimized pseudocode"
    );
}
...
fn test_large_contract_decompilation() {
    ...
    let additional_bytecode = vec![
        ...
        0x10, 0x11, 0x12, 0x13, 0x14, 0x15, // More arithmetic operations
        0x93, 0x94, 0x95, 0x96, 0x97, // Additional control flow
    ...
    assert!(
        result.is_ok(),
        "Large contract decompilation should succeed"
    );
    assert!(
        duration.as_secs() < 10,
        "Decompilation should complete in reasonable time"
    );

    let decompiled = result.unwrap();
    assert!(
        decompiled.instructions.len() > 10,
        "Should have many instructions"
    );
    assert!(
        !decompiled.pseudocode.is_empty(),
        "Should generate substantial pseudocode"
    );
}
...
fn test_memory_usage_patterns() {
    ...
    for (name, sample_nef) in test_cases {
        ...
        assert!(
            !decompiled.pseudocode.is_empty(),
            "Should generate pseudocode for {}",
            name
        );
    }
}
...
fn test_concurrent_decompilation() {
    ...
    for handle in handles {
        ...
        assert!(
            !decompiled.pseudocode.is_empty(),
            "Thread {} should generate pseudocode",
            thread_id
        );
    }
}
