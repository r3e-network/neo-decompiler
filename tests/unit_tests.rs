...
mod nef_parser_tests {
    ...
    fn test_parse_minimal_nef() {
        ...
        assert!(!nef_file.bytecode.is_empty(), "Bytecode should not be empty");
    }
...
mod manifest_parser_tests {
    ...
    fn test_parse_nep17_manifest() {
        ...
        assert!(parsed_manifest.supported_standards.contains(&"NEP-17".to_string()));
        assert!(!parsed_manifest.abi.methods.is_empty());
        
        // Check for required NEP-17 methods
        let method_names: Vec<_> = parsed_manifest.abi.methods.iter()
...
mod disassembler_tests {
    ...
    fn test_disassemble_simple_bytecode() {
        ...
        assert!(result.is_ok(), "Should successfully disassemble simple bytecode");
        ...
        assert!(instructions.iter().any(|i| format!("{:?}", i.opcode).contains("PUSH")));
    }
    ...
    fn test_disassemble_complex_control_flow() {
        ...
        assert!(result.is_ok(), "Should successfully disassemble complex bytecode");
...
mod ir_lifter_tests {
    ...
    fn test_lift_simple_instructions() {
        ...
        assert!(!ir_function.basic_blocks.is_empty(), "Should have basic blocks");
    }
        ...
        match result {
            Ok(ir_function) => {
                ...
                assert!(ir_function.basic_blocks.is_empty() || 
                        ir_function.basic_blocks.len() == 1);
            },
    ...
    fn test_lift_control_flow_instructions() {
        ...
        let result = lifter.lift_to_ir(&instructions);
        assert!(result.is_ok(), "Should successfully lift control flow to IR");
        
        let ir_function = result.unwrap();
        
        // Should create multiple basic blocks due to control flow
        assert!(ir_function.basic_blocks.len() >= 2, 
...
mod pseudocode_generator_tests {
    ...
    fn test_generate_simple_pseudocode() {
        ...
        assert!(pseudocode.contains("function") || 
                pseudocode.contains("main") || 
                pseudocode.len() > 10,
    ...
    fn test_generate_empty_function() {
        ...
        match result {
            Ok(pseudocode) => {
                assert!(!pseudocode.is_empty(), "Should generate at least minimal structure");
            },
            Err(_) => {} // Acceptable to fail on empty input
        }
    }
...
mod config_tests {
    ...
    fn test_default_config() {
        ...
        assert!(config.optimization_level <= 3, "Optimization level should be reasonable");
    }
...
mod error_handling_tests {
    ...
    fn test_error_chain_preservation() {
        ...
        let result: Result<(), DecompilerError> = Err(
            DecompilerError::ParseError("Test error".to_string())
        );
